...
def get_petrinet_cytoscape_data_from_model(model_id: int) -> dict:
    """
    Convert a stored PNML model into Cytoscape-compatible JSON data.

    Parameters
    ----------
    model_id : int
        ID of the DiscoveredProcessModel

    Returns
    -------
    dict
        Dictionary containing nodes and edges formatted
    ...
    # Convert PNML string → Petri Net object
    net, initial_marking, final_marking = pnml_string_to_petri_net(pnml_string)
    ...
    logger.info(f"Converted Petri net (model {model_id}) to Cytoscape JSON.")
...
def get_dataframe_stats(model_id: int) -> dict:
    """
    Get stats of the model's event log.
    ...
    -------
    dict
        Dictionary containing raw and cleaned stats of dataframe
    ...
    data = event_log.meta_info

    from log_preprocess import LogPreprocessor
    
    preprocessor = LogPreprocessor(df=event_log.uploaded_file.file.path)

    return {
        "data": data,
        # "cleaned_file_path": event_log.cleaned_file_path,
        "raw_file_path": str(event_log.uploaded_file.file.path),
        # "describe": preprocessor.describe()
    }
