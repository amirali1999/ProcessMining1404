...
def process_job(job_id):
    ...
    4. 🆕 Create EventLog and DiscoveredProcessModel in database
    5. Save output
    6. Update job status
    
    🔗 Integration Note:
    After this function completes successfully:
    - EventLog is created in uploads_eventlog table
    - DiscoveredProcessModel is created in discovery_discoveredprocessmodel table
    - Both can be accessed via API or ORM:
        event_log = EventLog.objects.get(name=project_name)
        model = DiscoveredProcessModel.objects.get(event_log=event_log)
    """
    job = None
    try:
        ...
        if output_pnml_path and os.path.exists(output_pnml_path):
            with open(output_pnml_path, 'r', encoding='utf-8') as f:
                pnml_content = f.read()
            
            # 🆕 Calculate statistics from DataFrame
            case_col = 'case:concept:name' if 'case:concept:name' in df.columns else df.columns[0]
            activity_col = 'concept:name' if 'concept:name' in df.columns else df.columns[1]
            
            num_cases = df[case_col].nunique() if case_col in df.columns else 0
            num_events = len(df)
            num_activities = df[activity_col].nunique() if activity_col in df.columns else 0
            
            # 🆕 Create EventLog if it doesn't exist
            from uploads.models import UploadedFile, EventLog
            
            # Check if an EventLog already exists for this job's file
            uploaded_file = None
            event_log = None
            
            try:
                # Try to find existing UploadedFile
                uploaded_file = UploadedFile.objects.filter(
                    file=job.original_file.name
                ).first()
                ...
                if not uploaded_file:
                    ...
                    uploaded_file = UploadedFile.objects.create(
                        ...
                        content_type='text/csv' if job.original_filename.endswith('.csv') else 'application/xml',
                        size_bytes=job.original_file.size if hasattr(job.original_file, 'size') else 0,
                    )
                
                # Try to find existing EventLog
                ...
                if not event_log:
                    # Create EventLog
                    file_type = 'csv' if job.original_filename.endswith('.csv') else 'xes'
                    event_log = EventLog.objects.create(
                        name=job.project_name or job.original_filename,
                        uploaded_file=uploaded_file,
                        file_type=file_type,
                        meta_info={
                            'uploaded_by': job.user.username,
                            'job_id': job_id,
                            'num_cases': num_cases,
                    ...
                    print(f"✅ Created EventLog (ID: {event_log.id}) for job {job_id}")
                else:
                    # Update metadata if it doesn't have stats
                    if not event_log.meta_info.get('num_cases'):
                        event_log.meta_info.update({
                            'num_cases': num_cases,
                            'num_events': num_events,
                            'num_activities': num_activities,
                        })
                        event_log.save()
                    print(f"✅ Using existing EventLog (ID: {event_log.id}) for job {job_id}")
                
            except Exception as e:
                print(f"⚠️  Warning: Could not create EventLog: {str(e)}")
                import traceback
                traceback.print_exc()
            ...
            if event_log:
                try:
                    # Compute Petri net statistics from PNML
                    from discovery.services import pnml_string_to_petri_net, compute_petri_net_stats
                    
                    net, im, fm = pnml_string_to_petri_net(pnml_content)
                    stats = compute_petri_net_stats(net)
                    
                    # Determine source version
                    source_version = 'cleaned' if job.cleaning_enabled else 'raw'
                    
                    # Create DiscoveredProcessModel
                    discovered_model = DiscoveredProcessModel.objects.create(
                        event_log=event_log,
                        discovered_by=job.user,
                        algorithm=job.mining_method,
                    ...
                    print(f"✅ Created DiscoveredProcessModel (ID: {discovered_model.id}) for job {job_id}")
                    print(f"   - Algorithm: {job.mining_method}")
                    print(f"   - Source: {source_version}")
                    print(f"   - Places: {stats['num_places']}, Transitions: {stats['num_transitions']}, Arcs: {stats['num_arcs']}")
                    
                    # Store model_id in job message for user reference
                    job.message = f'Model saved! (EventLog ID: {event_log.id}, Model ID: {discovered_model.id})'
                    
                except Exception as e:
                    print(f"⚠️  Warning: Could not create DiscoveredProcessModel: {str(e)}")
                    import traceback
                    traceback.print_exc()
...
class DiscoveredModelViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=['get'], url_path='cytoscape-data')        
    def petrinet_cytoscape_json_view(self, request, pk=None):
        """
        Get Cytoscape data as JSON.
        
        GET /api/discovered-models/{model_id}/cytoscape-data/
        """
        from django.http import JsonResponse
        from .services import get_petrinet_cytoscape_data_from_model
        data = get_petrinet_cytoscape_data_from_model(pk)
        return JsonResponse(data)
    
    @action(detail=True, methods=['get'], url_path='dataframe-stats')        
    def dataframe_stats_view(self, request, pk=None):
        """
        Stats of the event log.
        
        GET /api/discovered-models/{model_id}/dataframe-stats/
        """
        from .services import get_dataframe_stats
        
        data = get_dataframe_stats(pk)
