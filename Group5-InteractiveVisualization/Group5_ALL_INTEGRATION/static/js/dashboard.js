          ...
          <div class="workspace-placeholder">
            ...
            <div style="margin-top: 20px;">
              <div style="width: 200px; height: 4px; background: #e0e0e0; border-radius: 2px; overflow: hidden; margin: 0 auto;">
                <div style="height: 100%; background: #4285f4; width: ${data.progress || 0
					}%;"></div>
              </div>
              <p style="font-size: 11px; color: #999; margin-top: 8px;">${data.progress || 0
					}%</p>
            </div>
          </div>
        `;
...
// ================================================
// Visualize cytoscape data (interactive petrinet)
// ================================================
document.addEventListener("DOMContentLoaded", function () {
	...
	fetch(`/api/discovered-models/${modelId}/cytoscape-data/`)
		...
		.then(data => {
			...
			cy.fit();
			initializeCytoscapeZoom(cy);
		})
		.catch(error => console.error("Error loading Cytoscape data:", error));
});

// Function for initialize cytoscape zoom
function initializeCytoscapeZoom(cy) {

	const zoomInBtn = document.getElementById("cy-zoom-in-btn");
	const zoomOutBtn = document.getElementById("cy-zoom-out-btn");
	const zoomResetBtn = document.getElementById("cy-zoom-reset-btn");
	const zoomLevelDisplay = document.getElementById("cy-zoom-level");

	if (!zoomInBtn || !zoomOutBtn || !zoomResetBtn || !zoomLevelDisplay) {
		console.warn("Cytoscape zoom controls not found");
		return;
	}
...
document.addEventListener("DOMContentLoaded", function () {
	...
	fetch(`/api/discovered-models/${modelId}/dataframe-stats/`)
		.then(response => response.json())
		.then(data => {
			...
			// `
			// 	<h3>STATS</h3>
			// 	<!-- <p>${data.data.num_events}</p> -->
			// `;
		})
		.catch(error => console.error("Error loading dataframe stats:", error));
});